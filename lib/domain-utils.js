...
// DNR の requestDomains / excludedRequestDomains と同じく、指定ドメイン自身と
// そのサブドメインを一致扱いにする。
function matchesDomainRule(domain, ruleDomain) {
  const normalizedDomain = normalizeDomain(domain);
  const normalizedRuleDomain = normalizeDomain(ruleDomain);
  if (!normalizedDomain || !normalizedRuleDomain) return false;
  return normalizedDomain === normalizedRuleDomain ||
    normalizedDomain.endsWith(`.${normalizedRuleDomain}`);
}

// 対象ドメインへ適用されるルールのうち、最も具体的なドメインを返す。
// 例: example.com と api.example.com があれば、v1.api.example.com には後者を適用する。
function findMostSpecificDomainRule(domain, ruleDomains) {
  if (!Array.isArray(ruleDomains)) return null;
  return ruleDomains
    .filter(ruleDomain => matchesDomainRule(domain, ruleDomain))
    .sort((a, b) => b.split('.').length - a.split('.').length || b.length - a.length)[0] || null;
}
